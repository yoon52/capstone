...
const ProductDetail = ({ route }) => {
  ...
  useEffect(() => {
    const fetchProductDetail = async () => {
      try {
        const response = await fetch(`http://192.168.219.190:4000/products/detail/${productId}`);
        if (response.ok) {
  ...
  const handleChatButtonClick = async () => {
    try {
      const response = await fetch(`http://192.168.219.190:4001/api/chat-rooms?productId=${productId}&userId=${userId}`);
      if (response.ok) {
      ...
      const createResponse = await fetch('http://192.168.219.190:4001/api/chat-rooms', {
        method: 'POST',
        headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <Image style={styles.image} source={{ uri: `http://192.168.219.190:4000/uploads/${product.image}` }} />
