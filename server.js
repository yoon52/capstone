...
app.get('/oauth/kakao/callback', async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보를 데이터베이스에서 찾거나 저장
    const userId = profileData.id.toString(); // Kakao의 경우 사용자 ID는 숫자형이므로 문자열로 변환
...
app.get('/users', async (req, res) => {
  try {
    ...
    const query = `SELECT id, password, email, department, grade, admin FROM users WHERE admin != 'admin' AND admin != 'approved'`;
...
// 모든 승인된 사용자 정보 가져오는 엔드포인트
app.get('/approved-users', async (req, res) => {
  try {
    // 모든 승인된 사용자 정보를 가져오는 쿼리 실행
    const query = `SELECT id, password, email, department, grade, admin FROM users WHERE admin = 'approved'`;
    const [rows] = await pool.query(query);
