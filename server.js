...
const pathId = require('path'); // path 모듈을 추가로 가져옵니다.
const fetch = require('node-fetch');

// Google Cloud Vision 클라이언트 생성
const client = new vision.ImageAnnotatorClient({
  keyFilename: '/home/ubuntu/google_key/visiontest-418301-63ce6d68a706.json'
});
...
app.post('/api/verify', async (req, res) => {
  try {
    ...
    // 클라이언트로부터 이미지 URL과 사용자 ID(학번)를 받아옵니다.
...
async function extractNumbersFromImage(imageBuffer) {
...
// 사용자 승인 상태 업데이트 엔드포인트
app.put('/users/:id/approval', async (req, res) => {
  const userId = req.params.id; // 업데이트할 사용자의 아이디
  const { approvalStatus } = req.body; // 업데이트할 승인 상태

  try {
    // 사용자 승인 상태 업데이트 쿼리 실행
    const updateQuery = `UPDATE users SET admin = ? WHERE id = ?`;
    await pool.query(updateQuery, [approvalStatus, userId]);
